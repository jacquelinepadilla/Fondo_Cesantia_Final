...
public class cobros_realizados_servicios {
    private final String cobros_realizados = "cobros_realizados";  
  
    public void guardar(Connection conexion, cobros_realizados cobros) throws SQLException{
        try{
            PreparedStatement consulta;
            if(cobros.getId_Cobros() != 0){ 
                consulta = conexion.prepareStatement("INSERT INTO " + this.cobros_realizados + "(Id_Cobros, periodo, importe_total_capital, interes) VALUES(?,?,?,?)");
    ...
    public List<cobros_realizados> recuperarTodas(Connection conexion) throws SQLException{
        ...
        try{
            PreparedStatement consulta = conexion.prepareStatement("SELECT Id_Cobros, periodo, importe_total_capital, interes  FROM " + this.cobros_realizados + " ORDER BY Id_Cobros");
            ResultSet resultado = consulta.executeQuery();
            while(resultado.next()){
